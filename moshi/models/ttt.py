...
class TTTCache:
    ...
    def __init__(self, model, batch_size: int):
        ...
        for layer_idx in range(config.num_hidden_layers):
            for name in self.ttt_param_names:
                ...
                tiled_weight = torch.tile(weight.unsqueeze(0), (batch_size,) + (1,) * weight.dim()).to(
                    device=model.device, 
            ...
            if config.pre_conv:
                self.conv_states_dic["pre_conv"][layer_idx] = torch.zeros(
                    ...
                    config.conv_kernel,
                    device=model.device,
                    # Ensure consistency with model dtype
    ...
    def update(self, py_tree, layer_idx, seq_len):
        ...
        if seq_len % self.mini_batch_size == 0:
            ...
            for name in self.ttt_param_names:
                # Check dtypes before copying
                source_dtype = py_tree[f"{name}_states"].dtype
                target_dtype = self.ttt_params_dict[f"{name}_states"][layer_idx].dtype
                if source_dtype != target_dtype:
                    print(f"WARNING TTTCache.update: dtype mismatch for {name}_states - source: {source_dtype}, target: {target_dtype}")
                    # Ensure consistent dtype by explicitly converting
                    self.ttt_params_dict[f"{name}_states"][layer_idx].copy_(py_tree[f"{name}_states"].to(target_dtype))
                else:
            ...
            if (seq_len + self.seqlen_offset) % self.mini_batch_size == 0:
                ...
                for name in self.ttt_param_names:
                    # Check dtypes before copying
                    source_dtype = py_tree[f"{name}_grad"].dtype
                    target_dtype = self.ttt_params_dict[f"{name}_grad"][layer_idx].dtype
                    if source_dtype != target_dtype:
                        print(f"WARNING TTTCache.update: dtype mismatch for {name}_grad - source: {source_dtype}, target: {target_dtype}")
                        # Ensure consistent dtype by explicitly converting
                        self.ttt_params_dict[f"{name}_grad"][layer_idx].copy_(py_tree[f"{name}_grad"].to(target_dtype))
                    else:
    ...
    def log_ttt_state_to_wandb(self):
        """Log overall TTT state metrics to wandb.
        ...
        try:
            ...
            for layer_idx in available_layers:
                ...
                if "W2_states" in self.ttt_params_dict and self.ttt_params_dict["W2_states"][layer_idx] is not None:
                    avg_w2_norm += torch.norm(self.ttt_params_dict["W2_states"][layer_idx]).item()
                    avg_b2_norm += torch.norm(self.ttt_params_dict["b2_states"][layer_idx]).item()
            
            if layer_count > 0:
                avg_w1_norm /= layer_count
                avg_b1_norm /= layer_count
                
                metrics = {
                    "TTT_Avg_W1_Norm": avg_w1_norm,
                    "TTT_Avg_b1_Norm": avg_b1_norm,
                    "TTT_SeqLen_Offset": self.seqlen_offset,
                    "Step": self.seqlen_offset
                }
                
                if "W2_states" in self.ttt_params_dict:
                    avg_w2_norm /= layer_count
                    avg_b2_norm /= layer_count
                    metrics.update({
                        "TTT_Avg_W2_Norm": avg_w2_norm,
                        "TTT_Avg_b2_Norm": avg_b2_norm
                    })
...
class TTTLinear(TTTBase):
    ...
    def ttt(
        ...
        def compute_mini_batch(params_dict, inputs):
            ...
            if WANDB_AVAILABLE and last_mini_batch_params_dict is not None:
                try:
                    ...
                    b1_change = torch.norm(b1_last - params_dict["b1_states"]).item()
                    
                    # Calculate reconstruction loss (L2 norm of the difference between target and prediction)
        ...
        if cache_params is not None:
            ...
            # Remove the wandb logging code from here as it's now inside compute_mini_batch
        ...
        def compute_mini_batch(params_dict, inputs):
            ...
            if WANDB_AVAILABLE and last_mini_batch_params_dict is not None:
                try:
                    ...
                    w1_change = torch.norm(W1_last - params_dict["W1_states"]).item()
                    b1_change = torch.norm(b1_last - params_dict["b1_states"]).item()
                    w2_change = torch.norm(W2_last - params_dict["W2_states"]).item()
                    b2_change = torch.norm(b2_last - params_dict["b2_states"]).item()
...
class TTTModel(TTTPreTrainedModel):
    ...
    @staticmethod
    def init_wandb_logging(project_name="ttt-monitoring", run_name=None, config=None):
        """Initialize Weights & Biases logging for TTT model.
        ...
        try:
            ...
            print(f"Initialized wandb logging for project {project_name}")
            return True
        except Exception as e:
